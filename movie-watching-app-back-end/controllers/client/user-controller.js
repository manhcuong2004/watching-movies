...
module.exports.loginPost = async (req, res) => {
    try {
        ...
        res.cookie("token", newToken, {
            httpOnly: true,
            secure: false, // ⚠️ Đổi `false` nếu đang chạy trên localhost
            sameSite: "Lax", // Hoặc "None" nếu dùng HTTPS với CORS
        });
        res.status(200).json({ message: "Đăng nhập thành công!", newToken });
    } catch (error) {
        console.error("Lỗi server:", error);
        res.status(500).json({ error: "Lỗi server, vui lòng thử lại!" });
    }
